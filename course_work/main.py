...
def gaussSeidel_tStep():
    ...
    while not converge:
        ...
        for f in range(1, F_N - 1):
            U[R_N-1][f] = (alpha*(Uold[R_N - 2][f] - 2*dR*out + Uold[R_N-2][f]) + beta/R[R_N-1]*(Uold[R_N - 2][f] - 2*dR*out - Uold[R_N-2][f]) + gamma /R[R_N-1]/R[R_N-1]*(U[R_N-1][f+1] + U[R_N-1][f-1]) + Uold[R_N-1][f])/(1+2*alpha+2/R[R_N-1]/R[R_N-1]*gamma)
        ...
        for f in range(1, F_N - 1):
            U[0][f] = (alpha*(U[1][f] + U[1][f] - 2*dR*inner) + beta/R[0]*(U[1][f] - U[1][f] - 2*dR*inner) + gamma /R[0]/R[0]*(U[0][f+1] + U[0][f-1]) + Uold[0][f])/(1+2*alpha+2/R[0]/R[0]*gamma)
        ...
        for r in range(1, R_N - 1):
            ...
            for f in range(1, F_N - 1):
                U[r][f] = (alpha*(U[r+1][f] + Uold[r-1][f]) + beta/R[r]*(U[r+1][f] - Uold[r-1][f]) + gamma /R[r]/R[r]*(U[r][f+1] + U[r][f-1]) + Uold[r][f])/(1+2*alpha+2/R[r]/R[r]*gamma)
